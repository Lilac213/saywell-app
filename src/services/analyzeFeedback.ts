...
`;
  ...
  const prompt = `你是一个用户画像分析专家。用户提供了针对AI回复的反馈，请从中提炼关键信息并更新用户画像。
    ...
    {
      "personality_traits": {
        // ⚠️ 严禁使用英文 Key！必须使用中文描述特征。
        // 正确示例："直爽": true, "幽默感": "高"
        // 错误示例："direct": true
        ...现有的性格特征（保留中文 Key），
        ...根据反馈更新或补充的特征（必须使用中文 Key）
      },
      "language_habits": {
        // ⚠️ 严禁使用英文 Key！必须使用中文描述习惯。
        // 正确示例："常用语气": "轻松", "标点习惯": "简约"
        // 错误示例："tone": "casual"
        ...现有的语言习惯（保留中文 Key），
        ...根据反馈更新或补充的习惯（必须使用中文 Key）
      },
      "background_story": "更新后的背景故事（必须使用中文。如涉及性格和回复偏好的重大调整，请在此记录）"
    }`;
  ...
  console.log('Qwen API Response:', content);
  ...
  // 更新用户画像
  const updatedProfile = await userProfileApi.update(userProfileId, {
