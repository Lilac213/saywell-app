...
要求：
1. 仔细识别截图中的头像位置，准确区分发言者
2. 从右侧头像的消息学习用户的真实语言风格（这比问卷更准确）
3. 综合分析左侧头像的所有消息内容，可能包含多个问题或话题
4. 针对左侧头像的内容生成回复，确保回答完整、得体
5. 结合用户画像和截图中观察到的实际语言习惯
6. 特别注意用户的语言习惯：
   - 标点符号使用习惯（观察右侧消息中的实际使用）
   - 断句方式（一条长消息 vs 多条短消息）
   - 常用词汇和口头禅（从右侧消息中提取）
   - emoji使用频率和风格（观察实际使用情况）
   - 句子长度偏好（观察右侧消息的长度）
7. 如果有历史选择记录，学习用户的偏好模式
8. 生成4个不同风格的回复选项：
   - 前3个是具体的回复内容（严格模仿用户在右侧消息中的风格）
   - 第4个是"以上都不太合适"，用于收集用户反馈

请以JSON格式返回，格式如下：
{
  "extracted_text": "从截图中提取的聊天文本，格式：[对方]: 左侧头像消息内容\\n[用户]: 右侧头像消息内容",
  "context_analysis": "对聊天上下文的分析，包括：1) 对方说了什么（左侧头像）2) 用户之前怎么说的（右侧头像）3) 用户的实际语言风格观察",
  "intent_analysis": "分析对方想表达什么、期待什么样的回复、对话的情绪和氛围",
  "user_style_observation": "从右侧头像消息中观察到的用户实际语言风格（标点、断句、emoji、常用词等）",
  "replies": [
    {
      "text": "回复内容1（严格模仿右侧消息中用户的风格，回答对方的所有问题）",
      "tone": "回复风格",
